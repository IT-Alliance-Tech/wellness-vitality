 ...
 * 
 * @param {string} url - The Calendly booking URL
 */
const CalendlyEmbed = ({ url }) => {
  const [isLoading, setIsLoading] = useState(true);

  // Styling for Calendly widget via URL parameters
  const styledUrl = `${url}?hide_landing_page_details=1&hide_gdpr_banner=1&primary_color=3b3f69&text_color=3b3f69&button_color=ca1254`;
  ...
  return (
    <div className="relative w-full min-h-[700px] md:min-h-[850px] rounded-[24px] md:rounded-[40px] overflow-hidden bg-white border border-[#ca1254]/10 shadow-2xl shadow-indigo/5 group transition-all duration-500">
      ...
      {/* Background Pink Glow for Premium Feel */}
      ...
      {isLoading && (
        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-white/95 backdrop-blur-md transition-all duration-500">
          ...
          {/* Skeleton elements to mimic Calendly layout */}
      ...
      <iframe
        ...
        title="Wellness Vitality Booking"
        loading="lazy"
      />
    </div>
  );
